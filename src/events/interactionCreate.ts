...
async function handleButton(interaction: ButtonInteraction): Promise<void> {
  ...
  if (action === 'panel_open') {
    ...
    // Fast path: Check for form existence without full guild/member fetch yet
    if (categoryId) {
  ...
  if (action === 'panel_form') {
    ...
    try {
      ...
      await interaction.editReply({ embeds: [errorEmbed('Error', 'Failed to create ticket.')] });
    }
    return;
  }
  ...
  if (action === 'ticket_close') {
    ...
    // Fast path: Show modal immediately, check permissions on submit
    const modal = new ModalBuilder()
...
async function handleModal(interaction: ModalSubmitInteraction): Promise<void> {
  ...
  if (action === 'close_modal') {
    ...
    // Permission check inside the deferred state
    const member = interaction.guild.members.cache.get(interaction.user.id)
      ?? await interaction.guild.members.fetch(interaction.user.id);
