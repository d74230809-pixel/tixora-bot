...
export async function openTicket(options: {
  ...
  if (category?.max_open_per_user) {
    const res = await query('SELECT COUNT(*) FROM tickets WHERE guild_id = $1 AND opener_id = $2 AND category_id = $3 AND status = $4', [guild.id, member.id, categoryId, 'open']);
    if (parseInt(res.rows[0].count) >= category.max_open_per_user) throw new Error(`You already have ${res.rows[0].count} open ticket(s) in this category.`);
  }
  let channelName = `ticket-${member.user.username.toLowerCase().replace(/[^a-z0-9]/g, '')}`;
  if (category?.naming_scheme) channelName = category.naming_scheme.replace('{username}', member.user.username.toLowerCase().replace(/[^a-z0-9]/g, '')).replace('{id}', member.id).slice(0, 100);
  else channelName += `-${Date.now().toString(36)}`;
  ...
  try {
    const ticket = await createTicket({ guild_id: guild.id, channel_id: channel.id, opener_id: member.id, category_id: categoryId ?? null, priority_id: category?.default_priority_id ?? null, form_answers_json: formAnswers ?? null });
    ...
    if (category?.staff_roles_json?.length) {
      for (const roleId of category.staff_roles_json) {
        try { await channel.permissionOverwrites.create(roleId, { ViewChannel: true, SendMessages: true, ReadMessageHistory: true }); } catch (e) {}
      }
    }
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields({ name: '👤 Opener', value: `<@${member.id}>`, inline: true }, { name: '📂 Category', value: category?.name ?? 'General', inline: true }, ...(priorityData ? [{ name: '⚡ Priority', value: priorityData.name, inline: true }] : []))
      .setTimestamp().setFooter({ text: 'Tixora Support • tixora.app' });
    ...
    const mainMessage = await channel.send({ content: pingContent.trim() || undefined, embeds: [embed], components: [row] });
    if ((category as any)?.delete_ping && pingContent.trim()) setTimeout(() => mainMessage.edit({ content: '' }).catch(() => {}), 5000);
    if (formAnswers && Object.keys(formAnswers).length > 0) {
      const answersEmbed = new EmbedBuilder().setColor(0x5865F2).setTitle('📋 Form Responses').addFields(Object.entries(formAnswers).map(([k, v]) => ({ name: k, value: v.slice(0, 1024), inline: false })));
      await channel.send({ embeds: [answersEmbed] });
    }
    return channel;
  } catch (err) { await channel.delete().catch(() => {}); throw err; }
}
...
export async function buildAndSendTranscript(ticket: Ticket, channel: TextChannel): Promise<void> {
  try {
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields({ name: 'Ticket ID', value: `\`\`\`${ticket.id}\`\`\``, inline: true }, { name: 'Opened by', value: `<@${ticket.opener_id}>`, inline: true }, { name: 'Transcript', value: `[View Transcript](${transcriptUrl})`, inline: true })
      .setTimestamp();
